...
use super::{
    ...
    section::{EldersInfo, MemberInfo, Section},
    MessageHash, NodeMessage,
};
...
pub enum Variant {
    ...
    /// User-facing message
    UserMessage(Vec<u8>),
    /// NodeMessage sent to newly joined node containing the necessary info to become a member of our
    /// section.
    NodeApproval {
        elders_info: Proven<EldersInfo>,
        member_info: Proven<MemberInfo>,
    },
    /// NodeMessage sent to all members to update them about the state of our section.
    Sync {
    ...
    BouncedUntrustedMessage(Box<NodeMessage>),
    ...
    /// NodeMessage exchanged for DKG process.
    DKGMessage {
    ...
    /// NodeMessage containing a single `Vote` to be accumulated in the vote accumulator.
    Vote {
        content: Vote,

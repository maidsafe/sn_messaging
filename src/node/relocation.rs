...
use threshold_crypto::{PublicKey, Signature as BlsSignature};
use tokio::sync::mpsc;
...
impl RelocateDetails {
    pub fn new(section: &Section, network: &Network, peer: &Peer, destination: XorName) -> Self {
        Self::with_age(
            section,
            network,
            peer,
...
/// NodeMessage with Variant::Relocate in a convenient wrapper.
#[derive(Clone, Eq, PartialEq, Debug)]
pub struct SignedRelocateDetails {
    /// Signed message whose content is Variant::Relocate
    signed_msg: NodeMessage,
}

impl SignedRelocateDetails {
    pub fn new(signed_msg: NodeMessage) -> Result<Self, Error> {
    ...
    pub fn signed_msg(&self) -> &NodeMessage {
        &self.signed_msg
    }
...
pub fn check(age: u8, churn_signature: &BlsSignature) -> bool {
